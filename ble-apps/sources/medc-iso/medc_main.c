...
static void medcDmCback(dmEvt_t *pDmEvt)
{
  ...
  {
    APP_TRACE_INFO0("got legacy scan event");
...
static void medcScanReport(dmEvt_t *pMsg)
{
  ...
  else
  {
    ...
    /* look for BIG info? */
    if ((pData = DmFindAdType(DM_ADV_TYPE_BIG_INFO, pMsg->scanReport.len,
                              pMsg->scanReport.pData)) == NULL)
    {
      // StartBigSync?
      // pData = DmFindAdType(DM_ADV_TYPE_16_UUID_PART, pMsg->scanReport.len,
...
//  DmScanStart();
  ...
  DmBigSyncStart(1,1,1,60000,1,0);
}
...
static void medcProcMsg(dmEvt_t *pMsg)
{
  ...
  switch(pMsg->hdr.event)
  {
    ...
    case DM_SCAN_START_IND:
    	APP_TRACE_INFO0("DM_SCAN_START_IND");
      medcScanStart(pMsg);
    ...
    case DM_BIG_SYNC_EST_FAIL_IND:
      APP_TRACE_INFO0("DM_BIG_SYNC_EST_FAIL_IND");
      break;

    case DM_BIG_SYNC_EST_IND:
      APP_TRACE_INFO0("DM_BIG_SYNC_EST_IND");
      break;

    case DM_BIG_SYNC_LOST_IND:
      APP_TRACE_INFO0("DM_BIG_SYNC_LOST_IND");
      break;

    case DM_BIG_SYNC_STOP_IND:
      APP_TRACE_INFO0("DM_BIG_SYNC_STOP_IND");
      break;
